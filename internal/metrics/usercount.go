...
// relayUsersCount is a gauge with an approximate number of relay users for the
// last 1 hour.
var relayUsersCount = promauto.NewGauge(prometheus.GaugeOpts{
	Name:      "users_last_hour_count",
	Namespace: namespace,
	Subsystem: subsystemRelay,
	Help:      "The approximate number of SNI relay users for the last 1 hour.",
})

// relayUsersDailyCount is a gauge with an approximate number of relay users for
// the last 24 hours.
var relayUsersDailyCount = promauto.NewGauge(prometheus.GaugeOpts{
	Name:      "users_last_day_count",
	Namespace: namespace,
	Subsystem: subsystemRelay,
	Help:      "The approximate number of SNI relay users for the last 24 hours.",
})
...
func (c *userCounter) updateCounters(
	...
	relayUsersCount.Set(float64(hourly))
	relayUsersDailyCount.Set(float64(daily))
}
...
// RelayUsersCountUpdate records a visit by ip and updates the values of the
// [relayUsersCount] and [relayUsersDailyCount] gauges every second.
func RelayUsersCountUpdate(ip netip.Addr) {
